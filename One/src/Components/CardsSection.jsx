      ...
      <div className='max-w-6xl mx-auto'>
            <div className='max-w-6xl mx-auto'>
                  <div className='flex items-center space-x-2'>
                                    ...
                                    <h2 className="text-3xl font-bold mb-4">Learn about ONE and what we offer
                                    </h2>
        ...
        Global Alliance of National Human Rights Institutions.
            ...
            <div className='grid md:grid-cols-2 gap-6'>
                  {features.map((f, idx) => (
                        <motion.div
                        key={f.title}
                        className='bg-gray-100 rounded-lg shadow hover:shadow-md'
                        custom={idx}
                        initial='hidden'
                        whileInView="visible"
                        viewport={{ once: true }}
                        variants={card}
                        >
                              <div className={`p-2 ml-2 mt-2 rounded-md w-fit mb-4 ${f.bgColor}`}>
                                    <div className={`${f.iconColor} text-2xl transition-transform duration-300 group-hover:scale-110 group-hover:rotate-2`}>{f.icon}</div>
                              </div>
                              <h3 className='font-semibold text-lg p-2'>{f.title}</h3>
                              <p className='text-gray-600 p-2'>{f.desc}</p>
                        </motion.div>
                  ))}
            </div>
      </div>
